...
def max_val(state : Board, alfa: int, beta: int, color : str): #obtem a lista de possiveis estados de tabuleiro e retorna a avaliacao otimista
    ...
    for s_state in succ_list:
        node_value = max(node_value, min_val(s_state, alfa, beta, color))
...
def min_val(state : Board, alfa: int, beta: int, color : str): #obtem a lista de possiveis estados de tabuleiro e retorna a avaliacao pessimista (otimiza para o oponente)
    ...
    for s_state in succ_list:
        node_value = min(node_value, max_val(s_state, alfa, beta, color))
...
def alpha_beta_pruning(state : Board, color: str): #obtem a lista de possiveis estados do tabuleiro e retorna o melhor avaliado

    succ_list = successors(state, color)
    if not succ_list:
        return (-1,-1)
    moves_dict = dict(zip(succ_list, state.legal_moves(color)))
    best_succ = max(succ_list, key= lambda x: min_val(x, MIN_SCORE, MAX_SCORE, color))
...
def make_move(the_board : Board, color : str):
	...
	"""
	return alpha_beta_pruning(the_board,color)
